...
private:
    ...
    static auto test_serialize( int ) -> decltype(
        serialize( std::declval<U&>(), std::declval<const V&>() ),
        std::true_type {} );
...
public:
    static constexpr bool value = decltype( test_serialize<Ar, T>( 0 ) )::value;
};
#else
template<typename Ar, typename T>
struct has_function_const_serialize<false, false, Ar, T> {
...
private:
    template<typename U, typename V>
    static auto test_serialize( int ) -> decltype(
        serialize( std::declval<U&>(), std::declval<V&>() ),
        std::true_type {} );
